...
class swapExampleA():
    def __init__(self, wallet: str, private_key: str, web3RpcUrl: str, 
        ...
        self.WETH = self.web3.to_checksum_address('0x82af49447d8a07e3bd95bd0d56f35241523fbab1')
    ...
    def swap(self):
        ...
        swap_txn = self.router_contract.functions.swapExactETHForTokens(
                0, # Минимальное количество USDC для получения (0 для любого количества)
                [self.WETH, self.token_to_buy], # Путь обмена [WMATIC, USDC] 
                self.wallet, # Адрес получателя
                int(time.time()) + 10000 # Дедлайн  
            ).build_transaction({
                'from': self.wallet,
                'value': self.web3.to_wei(self.buy_amount, 'ether'), # Количество MATIC для отдачи
                'gas': self.gas,
